...
//    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            resultSet = db.executeResultSetQuery("SELECT count(dateFound) as aantal, extract(month FROM dateFound) as maand from foundluggage  GROUP BY extract(month from dateFound)");

            while (resultSet.next()) {
                int count = resultSet.getInt("aantal");
                int month = resultSet.getInt("maand");
                ...
                series.getData().add(new XYChart.Data<>(mon, count));
